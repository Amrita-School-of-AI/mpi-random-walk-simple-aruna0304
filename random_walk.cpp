...
void walker_process()
{
    ...
    std::cout << "Rank " << world_rank << ": Walker finished in " << stps << " steps." << std::endl;
...
void controller_process()
{
    ...
    int number_walkers = world_size - 1;
    int msg;
    MPI_Status status;

    // 2. Loop that many times to receive a message from each walker.
    for (int i = 0; i < number_walkers; i++)
    {
    ...
    std::cout << "Controller: All " << number_walkers << " walkers have finished." << std::endl;
}
